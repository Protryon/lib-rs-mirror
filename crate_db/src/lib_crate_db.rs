...
impl CrateDb {
    ...
    pub async fn before_index_latest(&self, origin: &Origin) -> FResult<()> {
        self.with_write("before_index_latest", |tx| {
            ...
            mark_updated.execute(args).map_err(|e| Error::Db(e, "upd-l"))?;
            Ok(())
        }).await
    }
    ...
    pub async fn index_latest(&self, c: CrateVersionData<'_>) -> FResult<DbDerived> {
        ...
        self.with_write("insert_crate", |tx| {
            ...
            for CategoryCandidate {rank_weight, category_relevance, slug, explicit} in &categories {
                ...
                insert_category.execute(args).map_err(|e| Error::Db(e, "insert cat"))?;
                if *explicit {
    ...
    async fn gather_crate_keywords(&self, c: &CrateVersionData<'_>) -> Result<KeywordInsert, Error> {
        ...
        for (i, (w2, k)) in c.extracted_auto_keywords.iter().enumerate() {
            ...
            if let Some((other, wother)) = self.tag_synonyms.get(k) {
                // intentionally flipped around to prefer normalized
                insert_keyword.add(other, w, false);
                insert_keyword.add(k, w * (1. - wother as f64), false);
            } else {
                insert_keyword.add(k, w, false);
            }
        }
    ...
    pub fn path_in_repo_tx(conn: &Connection, repo: &Repo, crate_name: &str) -> FResult<Option<String>> {
        ...
        none_rows(get_path.query_row(args, |row| row.get(0))).map_err(|e| Error::Db(e, "path_in_repo"))
    }
